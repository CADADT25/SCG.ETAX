...
namespace SCG.CAD.ETAX.WEB.Controllers.Etax
{
    public class InboxInprogressController : Controller
    {
        ...
        public async Task<JsonResult> SearchInprogress(string jsonString)
        {
            ...
            try
            {
                ...
                var task = await Task.Run(() => ApiHelper.PostURI("api/InboxManagement/SearchInprogress", httpContent));
                if (task.STATUS)
                {
